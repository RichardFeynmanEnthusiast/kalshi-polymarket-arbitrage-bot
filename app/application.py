...
class DoubleTimeHFTApp:
    ...
    def __init__(self, shutdown_balance: Decimal, dependencies: AppDependencies):
        self.shutdown_balance = shutdown_balance
        self.logger = logging.getLogger(__name__)
        self._setup_dependencies(dependencies)
        self.markets_to_trade: List[Tuple] = []
    ...
    def check_balance(self):
        try:
            ...
            raise Exception(f"Failed to generate new wallets. Service stopping: {e}")
    ...
    async def stop(self):
        """Stops the orchestrator and all its tasks."""
        await self.fletcher_orchestrator.stop()

    async def reset(self):
        """Stops the current trading session and clears all markets."""
        self.logger.info("Resetting application state...")
        await self.stop()
        self.markets_to_trade = []
        self.logger.info("Application has been reset and is ready for new configuration.")

def create_app(enable_diagnostic_printer: bool = False) -> DoubleTimeHFTApp:
    ...
    return DoubleTimeHFTApp(shutdown_balance=settings.SHUTDOWN_BALANCE,
