...
from shared_infra.kalshi_clients.kalshi_base import KalshiBaseClient
...
class KalshiHttpClient(KalshiBaseClient):
    ...
    def get_event(self, ticker: str, params={}):
        """ Fetches a single event given a market ticker"""
        resp = self.get(self.events_url + f"/{ticker}", params=params)
        return resp
    ...
    def get_all_events(
        ...
        """
        Fetches all events (paginating through cursors)
        """
        all_events = []
        seen = set()
        params = {"status": status, "limit": limit, "with_nested_markets" : with_nested_markets}
    ...
    def get_all_markets(
        ...
        """
        Fetches all market data from the Kalshi API.
        """
        all_markets = []
        cursor = None
        ...
        while True:
            ...
            try:
                data = self.get(self.markets_url, params=params)
